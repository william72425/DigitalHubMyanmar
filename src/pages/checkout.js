...
export default function Checkout() {
  ...
  // Load product when id is available
  useEffect(() => {
    if (id) {
  ...
  const loadUserData = async (userId) => {
    try {
      ...
      calculatePrice(discountPercent, hasOrder, userDoc.exists() ? userDoc.data().first_purchase_discount_used : false);
      setDataLoaded(true);
      setLoading(false);
    } catch (error) {
      console.error('Error loading user data:', error);
      calculatePrice(0, false, false);
      setDataLoaded(true);
      setLoading(false);
    }
  };

  const calculatePrice = (promoDiscountPercent, hasOrder, discountUsed) => {
    if (!product) return;
    ...
    if (promoDiscountPercent > 0 && !hasOrder && !discountUsed) {
